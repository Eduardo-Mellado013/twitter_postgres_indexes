...
def _insert_tweets(connection, input_tweets):
    ...
    for tweet in input_tweets:
        ...
        users.append({
            ...
            'urls': remove_nulls(','.join(u['expanded_url'] for u in tweet['user'].get('entities', {}).get('url', {}).get('urls', []))),
        ...
        for u in tweet.get('extended_tweet', {}).get('entities', {}).get('urls', tweet['entities']['urls']):
            tweet_urls.append({
                'id_tweets': tweet['id'],
                'urls': remove_nulls(u['expanded_url'])
            })

        for m in tweet.get('extended_tweet', {}).get('entities', {}).get('user_mentions', tweet['entities']['user_mentions']):
        ...
        hashtags = tweet.get('extended_tweet', {}).get('entities', {}).get('hashtags', tweet['entities']['hashtags'])
        cashtags = tweet.get('extended_tweet', {}).get('entities', {}).get('symbols', tweet['entities']['symbols'])
        ...
        for m in tweet.get('extended_tweet', {}).get('extended_entities', {}).get('media', tweet.get('extended_entities', {}).get('media', [])):
            tweet_media.append({
                'id_tweets': tweet['id'],
                'urls': remove_nulls(m['media_url']),
                'type': m['type']
            })
